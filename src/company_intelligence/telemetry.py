...
the gateway (https://company-intelligence.builditwithai.xyz/e). Opt-out and privacy: see README."""
...
try:
    ...
    MCP_SERVER_VERSION = importlib.metadata.version("company-intelligence")
except Exception:
    MCP_SERVER_VERSION = "0.1.0"


def _telemetry_disabled() -> bool:
    if os.getenv("COMPANY_INTELLIGENCE_TELEMETRY", "true").lower() in ("false", "0", "off"):
        return True
...
INTERNAL_RUN = os.getenv("COMPANY_INTELLIGENCE_INTERNAL", "").lower() in ("1", "true", "yes")


def _init_anonymous_identity():
    try:
        config_dir = Path.home() / ".company_intelligence"
